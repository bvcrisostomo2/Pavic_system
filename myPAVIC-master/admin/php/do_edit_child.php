...
if(isset($_POST['edit_child'])) {
  ...
  if ($mysqli->query($sql) === TRUE) {
    $sql = "SELECT CONCAT(first_name,' ', middle_name, ' ', last_name) name FROM pavic_children WHERE child_id = ".$child_id;
    if ($result = $mysqli->query($sql)) {
      if($row = $result->fetch_assoc()){
        $name = $row['name'];

        $sql = "INSERT INTO pavic_admin_logs(admin_id, activity) VALUES ($admin_id, 'Edited Child: $name')";
        $mysqli->query($sql);
      }
    }
    header('location: ../advocate-profile.php?success=5&advocate_id='.$advocate_id);

  } else {
    // echo "Error: " . $sql . "<br>" . $mysqli->error;
    header('location: ../error.php?error=7&advocate_id='.$advocate_id.'&child_id='.$child_id);
  }
